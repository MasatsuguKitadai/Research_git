...
int drift(char date[], char angle[])
{
 // 変数宣言
    ...
    sprintf(filename_read,"result/%s/01_csv_reverse/%s_reverse.csv",  date, angle);
    sprintf(filename_csv, "result/%s/02_csv_drift/%s_drift.csv",   date, angle);
    sprintf(filename_dat, "result/%s/02_dat_drift/%s_drift.dat",   date, angle);
    printf("【%s】\n", angle);
