...
/*********************************   MAIN   *********************************/
int twogage_sub(char date[], int split, double phi_1, double phi_2, double phi_3, double phi_4, double voltage_net[])
{
    /*****************************************************************************/
...
int main()
{
    ...
    for (i = -1 * count; i < count + 1; i++)
    {
        ...
        for (j = -1 * count; j < count + 1; j++)
        {
            buf = j;
            phi_x2 = buf / 10;
            // phi_x2 = j;

            for (k = -1 * count; k < count + 1; k++)
            {
                buf = k;
                phi_y1 = buf / 10;

                for (l = -1 * count; l < count + 1; l++)
                {
                    buf = l;
                    phi_y2 = buf / 10;
                    ...
                    twogage_sub("1", split, phi_x1, phi_x2, phi_y1, phi_y2, voltage_net);

                    for (m = 0; m < split; m++)
                    {
                        if (voltage_net[m] > average * 1.015 || voltage_net[m] < average * 0.985)
                        {
                            n = 1;
                            break;
                        }
