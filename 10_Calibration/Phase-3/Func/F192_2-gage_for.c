...
int twogage_sub(char date[], int split, double phi_1, double phi_2, double voltage_net[])
{
    // ファイルの指定
    char filename_read_1[100];
    char filename_read_2[100];
...
int main()
{
    ...
    for (i = -5; i < 6; i++)
    {
        ...
        for (j = -5; j < 6; j++)
        {
            ...
            /*
                        twogage_sub("1", split, phi_x, phi_y, voltage_net);

                        for (k = 0; k < split; k++)
                        {
                            sum = voltage_net[k] + sum;
                            average = sum / split;
                            // printf("%lf\n", average);
                        }
                        ...
                        for (k = 0; k < split; k++)
                        {
                            if (voltage_net[k] > average * 1.03 || voltage_net[k] < average * 0.97)
                            {
                                l = 1;
                            }
                        }
