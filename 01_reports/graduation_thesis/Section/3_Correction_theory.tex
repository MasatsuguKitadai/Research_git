...
水槽座標系と座標系[1]の回転における補正理論を説明する．
ここでは，座標系のオフセットはない($\Delta x = 0$，$\Delta y = 0$)として考える．
上述の通り水槽座標系と座標系[1]について，以下のFig.のように回転角$\theta_{x}$，$\theta_{y}$を持つ．
...
以上の過程より，座標系[1]から水槽座標系への変換が可能である．
...
水槽座標系と座標系[1]の回転角および作用力測定装置のひずみセンサの取付角，
水槽座標系における出力電圧勾配を調べることができる．

\newpage
\subsection{座標系のオフセットにおける補正理論}

次に，水槽座標系と座標系[2]のオフセットの補正理論を説明する．
ここでは，回転角はない($\theta_x = 0$，$\theta_2 = 0$)として考える．
水槽座標系と座標系[2]の中心との位置関係にオフセット$\Delta x$，$\Delta y$を持つ．
ここで，作用力$F$を与えるとき，その作用線はオフセット$\Delta x$，$\Delta y$によって
水槽座標系の中心$o$を通ることはなく，座標系[2]の中心$o''$を通る．
