...
\begin{figure}[htbp]
	...
	\begin{center}
		...
		\subcaption{Coordinate systems relationship}
	\end{center}
\end{figure}
...
また，Fig.のように座標系を定めるとき，出力電圧勾配の理論値は，
抗力方向を$v_{x\;\mathrm{theory}}$，揚力方向を$v_{y\;\mathrm{theory}}$として，
以下のように与えられる．

\begin{align}
	v_{x\;\mathrm{theory}} &= A \sin\left(\omega t + \frac{3}{2}\pi\right) = A \cos\left(\omega t + \pi\right)\\
	v_{y\;\mathrm{theory}} &= A \sin\left(\omega t + \pi\right) = A \cos\left(\omega t + \frac{1}{2}\pi\right)\\
	A &= const \notag
\end{align}
...
上述の通り水槽座標系と座標系Aについて，以下のFig.のように回転角$\theta_{x}$，$\theta_{y}$を持つ．
...
\begin{figure}[htbp]
	...
	\begin{center}
		...
		\subcaption{Relationship between ($x-y$) and ($x'-y'$)}
	\end{center}
\end{figure}
...
\begin{table}[htbp]
	\begin{center}
		\subcaption{Test data conditions}
...
\begin{figure}[htbp]
	...
	\begin{center}
		...
		\subcaption{Simulated data [Case 1]}
	\end{center}
\end{figure}
...
\begin{table}[htbp]
	\begin{center}
		\subcaption{DFT result value [Case 1]}
		\begin{tabular}{|p{30mm}|p{20mm}|p{20mm}|}
			\hline
...
\begin{multicols}{2}
	\begin{figure_here}
		\begin{center}
			...
			\subcaption{DFT Spectrum (Drag) [Case 1]}
		\end{center}
	\end{figure_here}
	...
	\begin{figure_here}
		\begin{center}
			...
			\subcaption{DFT Spectrum (Lift) [Case 1]}
		\end{center}
	\end{figure_here}
\end{multicols}
...
式()，式()より算出した位相角を用いて位相差$\theta_{x \;\mathrm{test}}$，$\theta_{y \;\mathrm{test}}$を求める．

\begin{align}
	\theta_{x\;\mathrm{test}} & = \pi \cdot \frac{180}{\pi} - 165.000 = 15.000\;\mathrm{[deg]}          \\
	\notag                                                                                              \\
	\theta_{y\;\mathrm{test}} & = \frac{\pi}{2} \cdot \frac{180}{\pi} - 70.000 = 20.000\;\mathrm{[deg]}
\end{align}
...
\begin{table}[htbp]
	\begin{center}
		\subcaption{Corected result of test data [Case 1]}
...
\begin{figure}
	...
	\begin{center}
		...
		\subcaption{Corrected data (Drag) [Case 1]}
		\includegraphics[width=95mm]{../../02_workspace/result/rotation_tx=15.0_ty=20.0/plot/21/21-4_corrected_angle_lift.png}
		\subcaption{Corrected data (Lift) [Case 1]}
	\end{center}
\end{figure}

\newpage

Fig.，Fig.をみると算出された補正値すなわち水槽座標系における出力電圧勾配は，
...
水槽座標系と座標系Bの中心との位置関係にオフセット$\Delta x$，$\Delta y$を持つ．
ここで，作用力$F$を与えるとき，その作用線はオフセット$\Delta x$，$\Delta y$によって
水槽座標系の中心$o$を通ることはなく，座標系Bの中心$o''$を通る．
...
また，座標系Bにおいて，作用点$F(x'',y'')$の座標はオフセット$\Delta x$，$\Delta y$を用いて
以下のように表される．
...
\begin{table}[htbp]
	\begin{center}
		\subcaption{Test data conditions (2)}
		\begin{tabular}{|p{30mm}|p{20mm}|p{20mm}|}
			\hline
...
\begin{figure}[htbp]
	...
	\begin{center}
		...
		\subcaption{Simulated data [Case 4]}
	\end{center}
\end{figure}
\vskip \baselineskip
...
\begin{table}[htbp]
	\begin{center}
		\subcaption{Corected result of test data [Case 4]}
...
\begin{figure}[htbp]
	...
	\begin{center}
		...
		\subcaption{Corrected data [Case 4]}
	\end{center}
\end{figure}


Fig.をみると算出された補正値は理論曲線上に位置していることがわかる．
...
\begin{table}[htbp]
	\begin{center}
		\subcaption{Test data conditions (3)}
...
\begin{figure}[htbp]
	\begin{center}
		...
		\subcaption{interpolated [Case 7]}
	\end{center}
\end{figure}
...
\begin{table}[htbp]
	\begin{center}
		\subcaption{Specified rotation angle [Case 7]}
		\begin{tabular}{|p{30mm}|p{20mm}|p{20mm}|}
			\hline
...
補正理論[1]を適用した結果を以下のFig.,に示す．

\begin{figure}
	\footnotesize
	\begin{center}
		\includegraphics[width=95mm]{../../02_workspace/result/simulation_tx=10.0_ty=-5.0_dx=5.00_dy=-2.50/plot/21/21-4_summary.png}
		\subcaption{Corrected data [Case 7]}
	\end{center}
\end{figure}

Fig.をみると，理論曲線と補正値がおおよそ一致していることが考えられる．

\subsection{正味出力電圧による評価}
...
\begin{figure}[htbp]
	...
	\begin{center}
		...
		\subcaption{Net output voltage [Case 7]}
	\end{center}
\end{figure}

Fig.をみるとテストデータ[Case 7]における，正味出力電圧はおおよそ一定になることがわかる．
