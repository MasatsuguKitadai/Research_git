...
このとき，作用力測定装置から得られる電圧$V_{x'1}$，$V_{y'1}$は作用力$F_{x'1}$，$F_{y'1}$に起因するものである．
また，得られた出力電圧$V_{x'1}$，$V_{y'1}$から，ロードセルの出力電圧$V_1$を用いて
出力電圧勾配$v_{x'1}$，$v_{y'1}$を求めることができる．
したがって，正規座標系と座標系[1]の関係について，
$v_{x1}$と$v_{x'1}$および$v_{y1}$と$v_{y'1}$の関係を明らかにすれば良い，

\subsubsection{回転角$\theta_{x1}$，$\theta_{y1}$の算出}
...
\begin{align}
    ...
    & = \tan \theta_2\; \left(\frac{v_{x'1} \cos \theta_2 - v_{y'1} \sin \theta_1}{\sin \theta_1 \sin \theta_2 + \cos \theta_1 \cos \theta_2}\right) + \frac{|v_{y'1}|}{\cos \theta_2}
\end{align}
\vskip \baselineskip
...
\begin{align}
    v_x & = \frac{v_{x'1} \cos \theta_2 - v_{y'1} \sin \theta_1}{\sin \theta_1 \sin \theta_2 + \cos \theta_1 \cos \theta_2}                                                                             \\
    v_y & = - \frac{1}{\tan \theta_1} \; \left(\frac{v_{x'1} \cos \theta_2 - v_{y'1} \sin \theta_1}{\sin \theta_1 \sin \theta_2 + \cos \theta_1 \cos \theta_2}\right) + \frac{|v_{x'1}|}{\sin \theta_1} \notag  \\
        & = \tan \theta_2\; \left(\frac{v_{x'1} \cos \theta_2 - v_{y'1} \sin \theta_1}{\sin \theta_1 \sin \theta_2 + \cos \theta_1 \cos \theta_2}\right) + \frac{|v_{y'1}|}{\cos \theta_2}
\end{align}
\vskip \baselineskip
...
ここで，作用力$F_2$を与えるとき，その作用線はオフセット$\Delta x$，$\Delta y$によって
...
供試体の半径を$r$とするとき，作用点$F_2(x,y)$の座標は角度$\varphi$を用いて以下のように表すことができる．
...
また，座標系[2]において，作用点$F_2(x'',y'')$の座標はオフセット$\Delta x$，$\Delta y$を用いて
以下のように表される．
...
ロードセルから与えられる作用力$F_2$はすべて供試体に伝わることはなく，
